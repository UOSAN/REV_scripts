...
def main():
    ...
    study = 'REV'
    number_confiles = 12
    output_directory = os.path.join(os.path.sep, 'Users', 'kristadestasio', 'Desktop', study + '_scripts', 'fMRI', 'rx', 'prepost_analysis', task, 'scripts', 'confile_lists')
...
def check_confiles(confiles, subject_dir_fullpaths, output_directory, task):
    for subject_dir_fullpath in subject_dir_fullpaths:
        for confile in confiles:
            output_textfile = os.path.join(output_directory, confile[0:-4] + '.txt')
            error_textfile = os.path.join(output_directory, 'missing_' + confile[0:-4] + '.txt')
