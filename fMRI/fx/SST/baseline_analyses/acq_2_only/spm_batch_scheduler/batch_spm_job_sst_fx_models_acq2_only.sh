...
# Set subject list
SUBJLIST=`cat subject_list_sst_acq2_only.txt`
...
SCRIPT=${STUDY}/REV_scripts/fMRI/fx/SST/baseline_analyses/acq2_only/scripts/matlabbatch_job_sst_acq2_only.m
...
# Set output dir
if [ ! -d "${STUDY}/REV_scripts/fMRI/fx/SST/baseline_analyses/acq2_only/scripts/output_logs" ]; then
    mkdir -v "${STUDY}/REV_scripts/fMRI/fx/SST/baseline_analyses/acq2_only/scripts/output_logs"
fi
OUTPUTDIR=${STUDY}/REV_scripts/fMRI/fx/SST/baseline_analyses/acq2_only/scripts/output_logs

# make sid_batch dir if doesn't exist
if [ ! -d "${STUDY}/REV_scripts/fMRI/fx/SST/baseline_analyses/acq2_only/scripts/sid_batches" ]; then
    mkdir -v "${STUDY}/REV_scripts/fMRI/fx/SST/baseline_analyses/acq2_only/scripts/sid_batches"
fi

if [ ! -d "${STUDY}/REV_scripts/fMRI/fx/SST/baseline_analyses/acq2_only/scripts/sid_batches/matlab_job_sst" ]; then
    mkdir -v "${STUDY}/REV_scripts/fMRI/fx/SST/baseline_analyses/acq2_only/scripts/sid_batches/matlab_job_sst"
fi
...
if [ "${PROCESS}" == "slurm" ]; then 
	...
	parallel --verbose --results "${OUTPUTDIR}"/{}_${RESULTS_INFIX}_output -j${MAXJOBS} bash spm_job_sst.sh ${REPLACESID} ${SCRIPT} :::: subject_list_sst_acq2_only.txt
fi
