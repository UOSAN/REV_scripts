...
# Set subject list
SUBJLIST=`cat subject_list_react_acq2_only.txt`
...
SCRIPT=${STUDY}/REV_scripts/fMRI/fx/React/baseline_analyses/prc_analysis/acq2_only/scripts/matlabbatch_job_react_acq2_only.m
...
# Set output dir
if [ ! -d "${STUDY}/REV_scripts/fMRI/fx/React/baseline_analyses/prc_analysis/acq2_only/scripts/output_logs" ]; then
    mkdir -v "${STUDY}/REV_scripts/fMRI/fx/React/baseline_analyses/prc_analysis/acq2_only/scripts/output_logs"
fi
OUTPUTDIR=${STUDY}/REV_scripts/fMRI/fx/React/baseline_analyses/prc_analysis/acq2_only/scripts/output_logs

# make sid_batch dir if doesn't exist
if [ ! -d "${STUDY}/REV_scripts/fMRI/fx/React/baseline_analyses/prc_analysis/acq2_only/scripts/sid_batches" ]; then
    mkdir -v "${STUDY}/REV_scripts/fMRI/fx/React/baseline_analyses/prc_analysis/acq2_only/scripts/sid_batches"
fi

if [ ! -d "${STUDY}/REV_scripts/fMRI/fx/React/baseline_analyses/prc_analysis/acq2_only/scripts/sid_batches/matlab_job_react" ]; then
    mkdir -v "${STUDY}/REV_scripts/fMRI/fx/React/baseline_analyses/prc_analysis/acq2_only/scripts/sid_batches/matlab_job_react"
fi
...
if [ "${PROCESS}" == "slurm" ]; then 
	...
	do
		...
		if [ ! -d "${STUDY}/bids_data/derivatives/baseline_analyses/sub-${SUB}/fx/react/prc" ]; then
    		mkdir -v "${STUDY}/bids_data/derivatives/baseline_analyses/sub-${SUB}/fx/react/prc"
		fi
	...
	parallel --verbose --results "${OUTPUTDIR}"/{}_${RESULTS_INFIX}_output -j${MAXJOBS} bash spm_job_react.sh ${REPLACESID} ${SCRIPT} :::: subject_list_react_acq2_only.txt
fi
