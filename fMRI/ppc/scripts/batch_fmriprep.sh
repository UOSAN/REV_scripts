...
sbatch --export subid=${SUBID},sessid=${SESSID},study_dir=${study_dir},study=${study},container=${container} --job-name fmriprep --partition=long -n16 --mem=75G --time=20:00:00 -o "${study_dir}"/"${study}"/REV_scripts/fMRI/ppc/output/"${SUBID}"_"${SESSID}"_fmriprep_output.txt -e "${study_dir}"/"${study}"/REV_scripts/fMRI/ppc/output/"${SUBID}"_"${SESSID}"_fmriprep_error.txt job_fmriprep.sh
