...
experiment_name='MSS Tasks (Go/No-Go for the REV Study';
experiment_code='REV';  % this should be a short (2-3 letter) code for the experiment
...
experiment_notes='This is the script for the Go/No-Go Peer Cue task for the TASC study.';
...
% set parameters for scaling images

%standardImHeight = 400; % for re-scaling the images to be standard size
...
for i=1:size(Devices,2) % Code from Lauren (9/16/14) to get MSS to use the correct input device to recognize the trigger from the scanner
Devices(1,i).usageName
Devices(1,i).transport
Devices(1,i).product
end
%inputDevice=6
%inputDevice 'test1'
...
for i = 1:length(st),
    ...
    elseif st(i) == 'p'
        ...
        if f < i
            ...
            imagetex(i) = itex;
            imDims = Screen('Rect', imagetex(i)); % Vector [Left Top Right Bottom]
            imHeight=imDims(4)-imDims(2); % Bottom - Top
%            imDims = imDims/imHeight*standardImHeight; % Scale image so height is standardImHeight
            imrect = CenterRect(imDims,Screen('Rect',w)); % Create a centered rectangle for the resized image
%             Screen('DrawTexture',w,imagetex(i),[],imrect); %Draw image
    ...
    elseif st(i) == 'n' %super hacky trying to get different settings for image display of instructions and first crosshair KG 9/21/14
        if exist(stFile{i},'file') ~= 2 %if file doesn't exist, skip trial
            fprintf('Warning: your file %s does not exist on trial %d.\n',stFile{i},i);
            continue;
        end;
    ...
    if bg(i) == 'p'
        ...
        if f < i
            ...
            bg_imagetex(i) = itex;
             %offsets image by amount specified in hshift and vshift
            %imDims = Screen('Rect', bg_imagetex(i)); % Vector [Left Top Right Bottom]
            %imHeight=imDims(4)-imDims(2); % Bottom - Top
            %imDims = imDims/imHeight*standardImHeight; % Scale image so height is standardImHeight
            %imrect = CenterRect(imDims,Screen('Rect',w)); % Create a centered rectangle for the resized image
            %Screen('DrawTexture',w,bg_imagetex(i),[],imrect); 
...
for j = 1:number_of_trials,
    ...
    if st(i) == 'm' %primary stimulus is a movie        
        ...
        elseif st(i) == 'p' %primary stimulus is a picture
            displaycommand = sprintf('Screen(''DrawTexture'',w,imagetex(i),[],bRect{i});');
        
        elseif st(i) == 'n' %stimulus is instruction or first crosshair KG 9/21/14
            displaycommand = sprintf('Screen(''DrawTexture'',w,imagetex(i),[],bRect{i});');
