...
# NOTE: Currently doing list-wise deletion on this data frame (na.omit), but this is getting rid of a lot of data. However, princomp() won't work with NA's in the dataset. Need to figure out how to manage PCA with missing data. 

all_pca_vars <- cbind(scons_pca_items,bis_pca_items,bis_bas_pca_items) %>%
  na.omit(all_pca_vars)
...
# Unrotated PCA
pca <- princomp(all_pca_vars, cor=TRUE)
...
# DOUBLE CHECK THESE
survey$RC_drive <- pca.orthogonal$scores[,1]
survey$RC_pos_affect <- pca.orthogonal$scores[,2]
survey$RC_neg_affect <- pca.orthogonal$scores[,3]

# correlate these 3 components and make sure they correspond to the original corr matrix 
```
