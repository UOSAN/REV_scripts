...
probs_heroinL <- c("BSCS_H", "DUH-H", "HCQ_SF")
...
for (v in 1:length(Vices)) {
  ...
  for (i in p) {
    survey_data[i][is.na(survey_data[i]) & survey_data[e] == "no"] <- " " #change real NAs to 999
...
# Calculate total score
survey_data$ftnd_total <- rowSums(survey_data[ftnd_recode_cols]) # ftnd total score
