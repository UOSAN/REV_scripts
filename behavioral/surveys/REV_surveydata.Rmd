...
# Get subscale means
survey_data$bscs_a <- rowMeans(survey_data[,bscs_key_list[[1]]]) # BSCS Alcohol items 1-3
survey_data$bscs_c <- rowMeans(survey_data[,bscs_key_list[[2]]]) # BSCS Cocaine items 1-3
survey_data$bscs_h <- rowMeans(survey_data[,bscs_key_list[[3]]]) # BSCS Heroin items 1-3
survey_data$bscs_ma <- rowMeans(survey_data[,bscs_key_list[[4]]]) # BSCS Marijauana items 1-3
survey_data$bscs_me <- rowMeans(survey_data[,bscs_key_list[[5]]]) # BSCS Meth items 1-3
survey_data$bscs_p <- rowMeans(survey_data[,bscs_key_list[[6]]]) # BSCS Prescription Pills items 1-3
...
survey_data$acq_sf_r_total <- rowMeans(survey_data[acq_sf_r_cols]) # acq_sf_r_data total score
...
# Calculate total score 
survey_data$audit_total <- rowMeans(survey_data[audit_cols]) # audit total score
...
# Calculate total score 
survey_data$pacs_total <- rowMeans(survey_data[pacs_cols]) # pacs total score
...
# Calculate total score
survey_data$ccq_brief_total <- rowMeans(survey_data[ccq_brief_cols]) # ccq_brief total score
...
# Calculate total score
survey_data$hcq_sf_14_total <- rowMeans(survey_data[hcq_sf_14_cols]) # hcq_sf_14 total score
...
# Calculate total score
survey_data$mcq_sf_total <- rowMeans(survey_data[mcq_sf_cols]) # mcq_sf total score
...
# Calculate total score
survey_data$ceq_s_p_total <- rowMeans(survey_data[ceq_s_p_cols]) # ceq_s total score
...
# Calculate total score
survey_data$ftnd_total <- rowMeans(survey_data[ftnd_cols]) # ftnd total score
...
# Calculate total score
survey_data$qsu_total <- rowMeans(survey_data[qsu_cols]) # qsu total score
...
survey_data[,reverse_cols] = 6 - survey_data[,reverse_cols] # Reverse score the hcq_sf_14 items in reverse_cols

# Calculate total score
survey_data$debq_total <- rowMeans(survey_data[debq_cols]) # debq total score
...
# survey_data$fci_crave_total <- rowMeans(survey_data[fci_cols]) # fci craving total score
```
...
# Calculate total score
survey_data$food_intensity <- rowMeans(survey_data[food_intensity_cols]) 
...
# Calculate total score
survey_data$drugs_intensity <- rowMeans(survey_data[drugs_intensity_cols])
...
# Calculate total score
survey_data$alcohol_intensity <- rowMeans(survey_data[alcohol_intensity_cols])
...
# Calculate total score
survey_data$tobacco_intensity <- rowMeans(survey_data[tobacco_intensity_cols])
...
hrb_alcohol_vars <- cbind(hrb_alcohol_crave,hrb_alcohol_consumption)

## calculate averages for craving and consumption for ALCOHOL
survey_data$hrb_alcohol_crave_avg <- rowMeans(hrb_alcohol_crave)

survey_data$hrb_alcohol_consumption_avg <- rowMeans(hrb_alcohol_consumption)
...
hrb_drugs_vars <- cbind(hrb_drugs_crave,hrb_drugs_consumption)

## calculate averages for craving and consumption for DRUGS
survey_data$hrb_drugs_crave_avg <- rowMeans(hrb_drugs_crave, na.rm = TRUE)

survey_data$hrb_drugs_consumption_avg <- rowMeans(hrb_drugs_consumption, na.rm = TRUE)

# tobacco
hrb_tobacco_crave <- survey_data[,c("qsu_total_z")]
hrb_tobacco_consumption <- survey_data[,c("ftnd_total_z")]
hrb_tobacco_vars <- cbind(hrb_tobacco_crave,hrb_tobacco_consumption)

## calculate averages for craving and consumption for TOBACCO
survey_data$hrb_tobacco_crave_avg <- hrb_tobacco_crave #NOTE: only one measure

survey_data$hrb_tobacco_consumption_avg <- hrb_tobacco_consumption #NOTE: only one measure
...
hrb_food_consumption <- survey_data[,c("debq_total_z")]
hrb_food_vars <- cbind(hrb_food_crave,hrb_food_consumption)

## calculate averages for craving and consumption for FOOD
survey_data$hrb_food_crave_avg <- rowMeans(hrb_food_crave)

survey_data$hrb_food_consumption_avg <- hrb_food_consumption #NOTE: only one measure

# combine craving vars together and consumption vars together
hrb_crave_vars <- as.data.frame(lapply(survey_data[,c("hrb_alcohol_crave_avg","hrb_drugs_crave_avg","hrb_tobacco_crave_avg","hrb_food_crave_avg")], as.numeric))

hrb_consumption_vars <- as.data.frame(lapply(survey_data[,c("hrb_alcohol_consumption_avg","hrb_drugs_consumption_avg","hrb_tobacco_consumption_avg","hrb_food_consumption_avg")], as.numeric))

# create weighted averages for craving and consumption
hrb_crave_avg_weighted <- as.data.frame(weighted.mean(x = as.matrix(hrb_crave_vars), w = as.matrix(hrb_weights), na.rm = TRUE))
...
weight_food_measures <- function(x){
  new <- x*survey_data$weighting_var_food
  return(new)
}

# apply this function to the food variables
hrb_food_vars <- as.data.frame(lapply(hrb_food_vars, FUN = function(X) weight_food_measures(X)))

# Make a function to re-name variables with suffix "_w" to indicated that it is a weighted variable
weight_by_intensity <- function(x){
  new <- paste0(x,"_w")
  return(new)
}

# apply this function
colnames(hrb_food_vars) <- lapply(colnames(hrb_food_vars), weight_by_intensity)
...
# Average all weighted z-scored CRAVING measures into a single HRB CRAVING summary variable 
hrb_crave <- mean(???)

# Average all weighted z-scored CONSUMPTION measures into a single HRB CONSUMPTION summary variable 
hrb_consumption <- mean(???)
```
