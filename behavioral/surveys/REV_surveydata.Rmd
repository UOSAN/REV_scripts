...
survey_data$hrb_crave_summary <- rowSums(cbind(weighted_crave_vars_food_avg,weighted_crave_vars_drugs_avg,weighted_crave_vars_alcohol_avg,weighted_crave_vars_tobacco_avg), na.rm = TRUE)
...
### NOTE: Using rowSums to calculate the HRB summary variables puts 0's in for cases in which the data was missing (i.e. it was trying to sum a row of all NA's). I will have to either manually replace these 0's with NA's or use na.omit somewhere. However, we want to keep all 143 rows, so na.omit might not be an option, as this would actually remove rows with missing data from the data frame. 


# THIS WAS THE ORIGINAL CODE I WROTE TO CALCULATE THE HRB SUMMARY VARS, BUT I DON'T THINK THIS IS CORRECT. CHECK WITH ELLIOT!

# # Average all weighted z-scored CRAVING measures into a single HRB CRAVING summary variable 
# survey_data$hrb_crave_summary <- rowMeans(weighted_crave_vars, na.rm = TRUE)
# 
# # Average all weighted z-scored CONSUMPTION measures into a single HRB CONSUMPTION summary variable 
# survey_data$hrb_consumption_summary <- rowMeans(weighted_consumption_vars, na.rm = TRUE)
```
...
survey_data$bis_bas_bas_drive <- rowSums(survey_data[,bis_bas_key_list[[1]]]) # BAS Drive
survey_data$bis_bas_bas_fun <- rowSums(survey_data[,bis_bas_key_list[[2]]]) # BAS Fun Seeking
survey_data$bis_bas_bas_reward <- rowSums(survey_data[,bis_bas_key_list[[3]]]) # BAS Reward Responsiveness
survey_data$bis_bas_bas_total <- rowSums(survey_data[,c("bis_bas_drive", "bis_bas_fun", "bis_bas_reward")]) # BAS total
survey_data$bis_bas_bis_total <- rowSums(survey_data[,bis_bas_key_list[[4]]]) # BIS
...
# NOTE: Whoever entered the scale skipped "upps_p_47", so we need to rename the variables from "upps_p_48" on. 

# INCORRECT CODE:
# rename(x = c("upps_p_48","upps_p_49","upps_p_50","upps_p_51","upps_p_52","upps_p_53","upps_p_54","upps_p_55","upps_p_56","upps_p_57","upps_p_58","upps_p_59","upps_p_60"), replace = c("upps_p_47","upps_p_48","upps_p_49","upps_p_50","upps_p_51","upps_p_52","upps_p_53","upps_p_54","upps_p_55","upps_p_56","upps_p_57","upps_p_58","upps_p_59"))
# 
# rename(x = survey_data[upps_cols][47], replace = c("upps_p_48" = "upps_p_47"))
# 
# upps_incorrect_names <- survey_data[upps_cols][47:59]
# 
# plyr::rename(upps_incorrect_names, c("upps_p_48" = "upps_p_47"))
# 
# 
# names(survey_data[upps_cols][47:59]) <- c("upps_p_47","upps_p_48","upps_p_49","upps_p_50","upps_p_51","upps_p_52","upps_p_53","upps_p_54","upps_p_55","upps_p_56","upps_p_57","upps_p_58","upps_p_59")
# 

# colnames(survey_data)[colnames(survey_data) == "upps_p_48"] <- "upps_47"
# colnames(survey_data)[colnames(survey_data) == "upps_p_49"] <- "upps_48"
# colnames(survey_data)[colnames(survey_data) == "upps_p_50"] <- "upps_49"
# colnames(survey_data)[colnames(survey_data) == "upps_p_51"] <- "upps_50"
# colnames(survey_data)[colnames(survey_data) == "upps_p_52"] <- "upps_51"
# colnames(survey_data)[colnames(survey_data) == "upps_p_53"] <- "upps_52"
# colnames(survey_data)[colnames(survey_data) == "upps_p_54"] <- "upps_53"
# colnames(survey_data)[colnames(survey_data) == "upps_p_55"] <- "upps_54"
# colnames(survey_data)[colnames(survey_data) == "upps_p_56"] <- "upps_55"
# colnames(survey_data)[colnames(survey_data) == "upps_p_57"] <- "upps_56"
# colnames(survey_data)[colnames(survey_data) == "upps_p_58"] <- "upps_57"
# colnames(survey_data)[colnames(survey_data) == "upps_p_59"] <- "upps_58"
# colnames(survey_data)[colnames(survey_data) == "upps_p_60"] <- "upps_59"
