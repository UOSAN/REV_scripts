...
homeBase = '~/Dropbox/REV_repos' #this is the only thing you should need to change
...
for (v in 1:length(Vices)) {
  ...
  print(paste0(e, ": ", get(ViceLS[v]))) #display the problem and name of associated scales (my own sanity check)
  for (i in p) {
    survey_data[i][is.na(survey_data[i]) & survey_data[e] == "no"] <- NA #change real NAs to "NA"
    print(variable.names(survey_data[i]))
  }
} #print to double check that I've put the right variables together!
...
###### Drug Use History (DUH) *NOTE: Not going to use this as HRB outcome variable
...
# Isolate the ceq_s items
ceq_s_cols <- colnames(survey_data[(grepfun("ceq_s_",survey_data))])
survey_data[ceq_s_cols] <- lapply(survey_data[ceq_s_cols], as.numeric)

# Calculate total score
survey_data$ceq_s_total <- rowMeans(survey_data[ceq_s_cols]) # ceq_s total score
...
library(car)
survey_data$ftnd_1_recode <- recode(survey_data$ftnd_1, "1=0;2=1;3=2;4=3")
survey_data$ftnd_2_recode <- recode(survey_data$ftnd_2, "1=0;2=1")
survey_data$ftnd_3_recode <- recode(survey_data$ftnd_3, "2=0")
survey_data$ftnd_4_recode <- recode(survey_data$ftnd_4, "1=0;2=1;3=2;4=3")
survey_data$ftnd_5_recode <- recode(survey_data$ftnd_5, "1=0;2=1")
survey_data$ftnd_6_recode <- recode(survey_data$ftnd_6, "1=0;2=1")

# isolate the recoded items
ftnd_recode_cols <- colnames(survey_data[(grepfun("_recode",survey_data))])

# Calculate total score
survey_data$ftnd_total <- rowMeans(survey_data[ftnd_recode_cols]) # ftnd total score. NOTE: this is originally supposed to be a sum, but we are using a mean in order to be robust to missing data
...
# NOTE: fci_pi_2 actually corresponds to fci_pa_2 (How much do you like pasta?); fci_pi_2.0 should actually be fci_pi_2 (How much do you like pizza?)

# isolate fci crave items
fci_cols <- c("fci_ch_1", "fci_co_1", "fci_d_1", "fci_f_1", "fci_i_1", "fci_pa_1", "fci_pi_1")
