...
for sub in ${sublist[@]}; do
	for run in $runnum; do
		if [ $(ls "${datadir}"/"${sub}_REV_${task}${run}"*.mat | wc -l) -eq 1 ]; then
		echo ${sub} "run" ${run} "exists" >> $outputlog
		elif [ $(ls "${datadir}"/"${sub}_REV_${task}${run}"*.mat | wc -l) -gt 1 ]; then
			echo ${sub} "run" ${run} "duplicate" >> $errorlog
		elif [ $(ls "${datadir}"/"${sub}_REV_${task}${run}"*.mat | wc -l) -eq 0 ]; then
