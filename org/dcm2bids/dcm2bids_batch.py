...
for line in lines:
	...
	if os.path.isdir(subjectpath):
		...
		batch_cmd = 'module load singularity; sbatch --job-name dcm2bids_{subject} --partition=short --time 00:60:00 --mem-per-cpu=2G --cpus-per-task=1 -o {niidir}/{subject}_dcm2bids_output.txt -e {niidir}/{subject}_dcm2bids_error.txt --wrap="singulariy run -B {archivedir} -B {configdir} {image} -d {subjectpath} -s {wave} -p {subject} -c {configfile} -o {niidir}"'.format(archivedir=archivedir,wave=wave,configdir=configdir,configfile=configfile,subject=subject,niidir=niidir,subjectpath=subjectpath,group=group,image=image)
		# Submit the job
