...
for line in lines:
	...
	if os.path.isdir(subjectpath):
		...
		batch_cmd = 'sbatch --job-name dcm2bids_{subjectdir} --partition=short --time 00:60:00 --mem-per-cpu=2G --cpus-per-task=1 -o {logdir}/{subjectdir}_dcm2bids_output.txt -e {logdir}/{subjectdir}_dcm2bids_error.txt --wrap="/projects/{group}/shared/dcm2niix/build/bin/dcm2niix -o {niidir} {subjectpath}"'.format(group=group,logdir=logdir,subjectdir=subjectdir,wave=wave,subject=subject,niidir=niidir,subjectpath=subjectpath)
		# Submit the job
