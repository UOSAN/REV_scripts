...
# check for run string in correct place
			for file in os.listdir(fullpath):
				print(file)
				if file.split("_")[0] != glob.glob("run-[0-9]{2}") and file.split("_")[1] != glob.glob("run-[0-9]{2}") and file.split("_")[2] != glob.glob("run-[0-9]{2}") and file.split("_")[3] == glob.glob("run-[0-9]{2}"):
					print(file) 
