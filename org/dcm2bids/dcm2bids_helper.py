...
if os.path.isdir(dicomdir):
	...
	cmd = 'module load singularity; srun --job-name helper_{test_subject} --partition=short --time 00:60:00 --mem-per-cpu=2G --cpus-per-task=1 -o {logdir}/{test_subject}_helper_output.txt -e {logdir}/{test_subject}_helper_error.txt --wrap="singularity run -B {dicomdir} -B {test_subject} {image} dcm2bids_helper -d {dicomdir}/{test_subject} -o /projects/{group}/shared/{study}"'.format(dicomdir=dicomdir,test_subject=test_subject,niidir=niidir,group=group,image=image,study=study)
	# Submit the job
