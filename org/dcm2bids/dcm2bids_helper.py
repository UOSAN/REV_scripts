...
if os.path.isdir(dicomdir):
	...
	cmd = 'module load singularity; singularity run -B {dicomdir} -B {test_subject} {image} dcm2bids_helper -d {dicomdir}/{test_subject} -o /projects/{group}/shared/{study}'.format(dicomdir=dicomdir,test_subject=test_subject,niidir=niidir,group=group,image=image,study=study)
	# Submit the job
