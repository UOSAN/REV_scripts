...
dicomdir = os.path.join(os.sep, "projects", os.sep, "lcni", os.sep, "dcm", group, "Archive", study)

# Set directories
niidir = os.path.join(os.sep, "projects", group, "shared", study, "bids_data") # Where the niftis will be put
codedir = os.path.join(os.sep, "projects", group, "shared", study, gitrepo, "org", os.sep, "dcm2bids")  # Contains subject_list.txt, config file, and dcm2bids_batch.py
configfile = os.path.join(codedir, study + "_config.json")  # path to and name of config file
image = os.path.join(os.sep, "projects", group, "shared", os.sep, "containers", os.sep, "Dcm2Bids-master.simg")
